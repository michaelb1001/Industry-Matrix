...
if st.button("🚀 Generate AI Matrix"):
    with st.spinner("Thinking with GPT-4..."):
        ...
        if filtered.empty:
            ...
            def normalize_url(url, timeout=4):
                fallback_url = "https://www.hustlefund.vc/founders"
            ...
            subset['Formatted'] = subset.apply(
                lambda row: f"[{row['Portco Name']}]({normalize_url(row['Company URL'])}) – {row['Company Description']}",
                axis=1
            )
...
#     return "https://www.hustlefund.vc/founders"


# #streamlit run matrix_interface.py
# print(normalize_url("http://yana.com.mx/"))  # should return the full correct URL